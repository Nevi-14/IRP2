...
async generarListaGuiasEstadoINIT(camiones:GuiaEntrega[]){
  let data = [];


    for(let i =0; i < camiones.length; i++){
  
      data.push( this.generarGuiaEstadoINI(camiones[i]));
      if(i ==  camiones.length -1){
...
return guia;
   ...
   generarGuiaEstadoINI(camion:GuiaEntrega) {
   let i = this.gestionCamionesService.camiones.findIndex(truck => truck.idCamion == camion.idCamion);

   if(i >=0){

    let capacidad = camion.peso;
    let id = camion.idGuia;
    let guia = {
      idGuia:  id,
      guiaExistente: true,
      verificada: false,
      ...
      camion:{
        numeroGuia: id,
        chofer:this.gestionCamionesService.camiones[i].chofer,  
        idCamion: camion.idCamion,
        capacidad: capacidad,
        pesoRestante: capacidad - camion.peso,
        peso: camion.peso ,
        estado : camion.estado,
        HH : camion.HH,
        volumen:   camion.volumen,
        frio:this.gestionCamionesService.camiones[i].frio,
        seco:this.gestionCamionesService.camiones[i].seco,
        HoraInicio:'08:00',
        HoraFin:'20:00'
      },
      clientes:[],
...
if(i >=0){
  ...
  console.log('id', idGuia, 'factura', factura)
...
if(pesoActual+pesoFactura < capacidadCamion){
